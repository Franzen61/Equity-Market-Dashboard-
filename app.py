...
def percentile_of(series, value):
    """Return 0-100 percentile of value within series, ignoring NaN."""
...
def pct_color(pct, invert=False):
    """Color by percentile. invert=True: high pct = danger (VIX, PCR, TNX)."""
    if pct is None:
        return AMBER
    if invert:
...
def percentile_badge_html(pct, invert=False):
    """Mini bar + percentile label embedded in tile."""
    if pct is None:
        return ""
    ...
    return (
        ...
        f'</span>'
        f'<span style="color:{col};font-weight:700">{pct:.0f}° pct 1Y</span>'
        f'</div>'
    )

def tile(label, value, delta=None, color_class="", unit="", pill_label=None,
         pct=None, pct_invert=False):
    delta_html = ""
...
def compute_spy_vix_ratio(spy_series, vix_series, window=63):
    """
    SPY/VIX Normalized Regime Indicator.

    Rationale:
      SPY/VIX raw = price-to-fear ratio.  When SPY rises and VIX falls the
      ratio expands → risk-on.  When SPY drops and VIX spikes the ratio
      collapses → risk-off.

      A rolling z-score (window=63 ≈ 1 quarter) removes the secular uptrend
      in SPY and makes the signal stationary and mean-reverting.

      z > +1  → clear risk-on regime
      z < -1  → clear risk-off regime
      between → transitional / indeterminate

    Returns:
      z_norm  : z clipped to [-3,+3] then mapped to [0,100] — for gauge
      z       : raw z-score series — for bar chart
      raw     : SPY/VIX ratio — for historical chart
    """
...
with st.sidebar:
    ...
    st.markdown(
        ...
        ' → E-mini S&P500 → Volume &amp; OI → <b>MAR26 AT CLOSE</b></div>', unsafe_allow_html=True)
...
# Percentile calculations (always 2Y)
...
def score_spy_vix(z):
    """
    1 pt when regime is clearly risk-on (z > +1).
    0.5 pt in transitional zone.
    0 pt risk-off.
    Adds a regime-based dimension orthogonal to all other indicators.
    """
...
with tab1:
    ...
    with col_g:
        ...
        st.markdown(f"""
        <div style="background:#080e14;border:1px solid #1c2a3a;border-radius:4px;padding:10px 12px;margin-top:10px">
          <div style="font-size:0.55rem;letter-spacing:3px;color:#4a6070;text-transform:uppercase;margin-bottom:6px">Score Breakdown</div>
          <table style="width:100%;border-collapse:collapse">{_rows_html}</table>
          <div style="font-size:0.52rem;color:#4a6070;margin-top:6px;text-align:right">Totale: {total:.1f} / {max_score} pt</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_m:
        ...
        with c4:
            ...
            st.markdown(tile("VIX3M/VIX Ratio", skew_v, None, "blue", "", sp_s_, pct=pct_skew), unsafe_allow_html=True)
        with c5:
            ...
            st.markdown(tile("Put/Call Ratio", pcr_v, None, pcr_cc, "", pp, pct=pct_pcr, pct_invert=True), unsafe_allow_html=True)
        with c6:
        ...
        with c7:
            ...
            st.markdown(tile("HYG/LQD · Credit", hl_v, None, hl_c, "", hl_p, pct=pct_hl), unsafe_allow_html=True)
        with c8:
            if tnx_last:
                tnx_v  = f"{tnx_last:.2f}%"
        ...
        z_disp_str   = f"{spy_vix_z_last:+.2f}σ"    if spy_vix_z_last    is not None else "N/A"
        ...
        <div class="regime-panel">
          <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
            <div>
              <div style="font-size:0.58rem;letter-spacing:3px;color:#4a6070;text-transform:uppercase">Rolling 63d · Composite contributo 1/10</div>
              <div style="font-family:Syne;font-size:1.1rem;font-weight:700;color:{regime_color};margin-top:2px">
                {z_disp_str} · {regime_label}
              </div>
              <div style="font-size:0.62rem;color:#8ab0c8;margin-top:3px">{regime_desc}</div>
            </div>
            <div style="margin-left:auto;text-align:right">
              <div style="font-size:0.55rem;color:#4a6070;letter-spacing:2px">GAUGE NORM.</div>
              <div style="font-family:Syne;font-size:1.5rem;font-weight:700;color:{regime_color}">{norm_disp_str}</div>
              <div style="margin-top:4px">{signal_pill(pill_regime)}</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab2:
    ...
    st.info("⌨️  Aggiorna settimanalmente i valori nella sidebar → Barchart: $S5TH, $S5FI, $NDTH, $NDFI", icon="ℹ️")
    ...
    <div style="border:1px solid #1c2a3a;border-radius:4px;overflow:hidden">
      <table style="width:100%;border-collapse:collapse;background:#080e14">
        <thead><tr style="background:#0e1420">
        ...
        <tbody>{rows}</tbody>
        <tfoot><tr style="background:#0e1420">
          <td colspan="3" style="padding:8px 12px;font-size:0.62rem;color:#7a9ab0">BREADTH COMPOSITE</td>
          <td style="padding:8px 12px">
            <span style="background:{b_bg};color:{b_color};border:1px solid {b_color};padding:2px 10px;border-radius:2px;font-size:0.6rem;letter-spacing:2px">{b_label} ({breadth_score}/4)</span>
          </td>
        </tr></tfoot>
      </table>
    </div>
    ...
    _ad_img = st.file_uploader("Carica screenshot A/D Line (PNG/JPG)", type=["png","jpg","jpeg","webp"],
    ...
    if "ad_img_bytes" in st.session_state:
        ...
        <div style="border:1px solid #1c2a3a;border-radius:4px;overflow:hidden;margin-bottom:8px">
          <div style="background:#0e1420;padding:6px 12px;font-size:0.58rem;color:#7a9ab0;letter-spacing:2px;display:flex;justify-content:space-between">
            <span>📊 NYSE A/D LINE</span><span>Caricato: {_upload_date} · {_upload_name}</span>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        <div style="background:#0e1420;border:1px solid #1c2a3a;border-radius:4px;padding:32px;text-align:center">
          ...
          <div style="font-size:0.62rem;color:#7a9ab0;line-height:1.9">
            <b style="color:#8ab0c8">Fonti:</b> marketinout.com · stockcharts.com → $NYAD · barchart.com → NYSE Breadth
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab3:
    ...
    _scenario_map = {
        ("low",  "cont"): ("⚠️ EUFORIA",       "VIX basso e curva piatta: compiacenza massima.", AMBER),
    ...
    st.markdown(f"""
    <div style="background:#0a0f18;border:1px solid #1c2a3a;border-radius:4px;padding:14px 18px;margin:4px 0">
      <div style="font-size:0.58rem;letter-spacing:3px;color:#4a6070;margin-bottom:10px;text-transform:uppercase">VIX · VIX3M/VIX · Scenario Combinato</div>
      <div style="display:flex;gap:24px;flex-wrap:wrap">
        ...
        <div style="flex:2;min-width:280px">
          <table style="width:100%;border-collapse:collapse;font-size:0.58rem">
            <tr style="color:#4a6070">
              <td style="padding:2px 8px">VIX</td><td style="padding:2px 8px">VIX3M/VIX</td><td style="padding:2px 8px">SCENARIO</td>
            </tr>
            <tr style="background:{'#0e1a10' if _key==('low','cont')  else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">&lt; 15</td><td style="padding:3px 8px;color:#7a9ab0">&gt; 1.00</td><td style="padding:3px 8px;color:#f5a623">⚠️ Euforia</td>
            </tr>
            <tr style="background:{'#0e1a10' if _key==('mid','cont')  else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">15–25</td><td style="padding:3px 8px;color:#7a9ab0">&gt; 1.00</td><td style="padding:3px 8px;color:#f5a623">🟡 Nervosismo temporaneo</td>
            </tr>
            <tr style="background:{'#0e1a10' if _key==('high','cont') else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">&gt; 25</td><td style="padding:3px 8px;color:#7a9ab0">&gt; 1.00</td><td style="padding:3px 8px;color:#f5a623">🟡 Paura passeggera</td>
            </tr>
            <tr style="background:{'#1a0a0a' if _key==('mid','back')  else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">15–25</td><td style="padding:3px 8px;color:#7a9ab0">&lt; 1.00</td><td style="padding:3px 8px;color:#ff4d6d">🔴 Stress in aumento</td>
            </tr>
            <tr style="background:{'#1a0a0a' if _key==('high','back') else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">&gt; 25</td><td style="padding:3px 8px;color:#7a9ab0">&lt; 1.00</td><td style="padding:3px 8px;color:#ff4d6d">🔴🔴 Crisi acuta</td>
            </tr>
          </table>
          <div style="font-size:0.55rem;color:#4a6070;margin-top:6px;line-height:1.6">
            Riga evidenziata = scenario corrente · VIX3M/VIX &gt; 1 = contango · &lt; 1 = backwardation
          </div>
        </div>
      </div>
    </div>
    ...
    with c3:
        ...
        if pcr_barchart_val and pcr_barchart_puts and pcr_barchart_call:
            ...
            st.markdown(f"""
            <div style="font-size:0.62rem;color:#8ab0c8;border:1px solid #1c2a3a;padding:8px;border-radius:4px;margin-top:8px;line-height:1.9">
    ...
    with c4:
        if pcr_s is not None:
            ...
            st.markdown(f"""
            <div style="background:#0e1420;border:1px solid #1c2a3a;padding:20px;border-radius:4px;text-align:center;margin-top:20px">
              <div style="font-size:0.6rem;letter-spacing:3px;color:#7a9ab0">P/C RATIO OGGI (CSV)</div>
              <div style="font-family:Syne;font-size:3rem;font-weight:700;color:{color_pcr}">{pcr_barchart_val:.3f}</div>
            </div>""", unsafe_allow_html=True)
        else:
    ...
    <div style="font-size:0.65rem;color:#8ab0c8;border:1px solid #1c2a3a;padding:10px;border-radius:4px;line-height:1.8;margin-top:12px">
      ...
      <b>Bande grafici</b> = 25°/75° pct 2 anni
    </div>
    """, unsafe_allow_html=True)
    ...
    with c_hyg1:
        ...
        if hyg_lqd_last:
            ...
            st.markdown(f"""
            <div style="background:#0e1420;border:1px solid #1c2a3a;padding:12px;border-radius:4px;font-size:0.63rem;line-height:1.9">
              <b style="color:{hl_col}">{hl_label}</b><br>
              <span style="color:#8ab0c8">{hl_msg}</span><br><br>
              <span style="color:#4a6070">~0.60 crisi → ~1.02 picco · HYG=HY Corp · LQD=IG Corp</span>
            </div>""", unsafe_allow_html=True)
...
with tab4:
    ...
    with c1:
        ...
        st.markdown(
            ...
            '📌 Fonte: <a href="https://www.cmegroup.com/markets/equities/sp/e-mini-sandp500.volume.html" target="_blank" style="color:#00f5c4;font-weight:700;text-decoration:none;">CMEGroup.com → E-mini S&P500 → Volume &amp; OI</a>'
            ' → seleziona <b>MAR26 AT CLOSE</b></div>', unsafe_allow_html=True)
    ...
    with c2:
        ...
        st.markdown(
            '<div style="font-size:0.7rem;color:#8ab0c8;margin-bottom:10px;line-height:1.8">'
            '📌 Fonte: <a href="https://www.finra.org/rules-guidance/key-topics/margin-accounts/margin-statistics" target="_blank" style="color:#00f5c4;font-weight:700;text-decoration:none;">FINRA.org → Margin Statistics</a>'
            ' → aggiorna mensilmente (lag ~3-4 settimane)</div>', unsafe_allow_html=True)
    ...
    with c_r1:
        ...
        if tnx_last:
            ...
            st.markdown(
                ...
                f'<span style="color:#4a6070">Soglie: &lt;3.5% bull · 3.5–4.5% neutrale · &gt;4.5% bear</span></div>',
                unsafe_allow_html=True)
    ...
    with c_r2:
        ...
        if spread_2y10y is not None:
            ...
            st.markdown(
                ...
                f'<span style="color:#4a6070">Indicatore lento · non nel composite · segnala contesto recessivo</span></div>',
                unsafe_allow_html=True)
...
with tab5:
        ...
        <div style="flex:1;min-width:180px">
          <table style="font-size:0.6rem;border-collapse:collapse;width:100%">
            <tr style="color:#4a6070;letter-spacing:1px">
              <td style="padding:3px 8px">Z-Score</td><td style="padding:3px 8px">Regime</td>
            </tr>
            <tr style="background:{'#0a1a14' if spy_vix_z_last and spy_vix_z_last > 1.0 else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">&gt; +1.0σ</td>
              <td style="padding:3px 8px;color:#00f5c4">✅ RISK-ON · 1 pt composite</td>
            </tr>
            <tr style="background:{'#1a150a' if spy_vix_z_last and -1.0 <= spy_vix_z_last <= 1.0 else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">−1 → +1σ</td>
              <td style="padding:3px 8px;color:#f5a623">⚡ TRANSITIONAL · 0.5 pt</td>
            </tr>
            <tr style="background:{'#1a0a0a' if spy_vix_z_last and spy_vix_z_last < -1.0 else 'transparent'}">
              <td style="padding:3px 8px;color:#7a9ab0">&lt; −1.0σ</td>
              <td style="padding:3px 8px;color:#ff4d6d">⚠️ RISK-OFF · 0 pt</td>
            </tr>
          </table>
          <div style="font-size:0.55rem;color:#4a6070;margin-top:8px;line-height:1.6">
            Window 63d ≈ 1 trimestre<br>
            Cattura mean-reversion tra cicli<br>
            Non adatto a scalping
          </div>
        </div>
      </div>
    </div>
    ...
    with col_g2:
        ...
        st.markdown(f"""
        <div style="background:#0e1420;border:1px solid #1c2a3a;padding:12px;border-radius:4px;
                    font-size:0.65rem;line-height:2;text-align:center">
          <div style="color:#4a6070;font-size:0.55rem;letter-spacing:2px">Z-SCORE CORRENTE</div>
          <div style="font-family:Syne;font-size:2rem;font-weight:700;color:{regime_color}">
            {f"{spy_vix_z_last:+.2f}σ" if spy_vix_z_last is not None else "N/A"}
          </div>
          <div style="margin-top:4px">{signal_pill(pill_regime2)}</div>
          <div style="font-size:0.55rem;color:#4a6070;margin-top:8px">
            Composite score: {_sc_spy_vix:.1f}/1.0
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_ch:
        if spy_vix_z_disp is not None and len(spy_vix_z_disp) > 5:
            colors_z = [CYAN if v > 1.0 else (RED if v < -1.0 else AMBER)
    ...
    if spy_vix_z_disp is not None and len(spy_vix_z_disp) > 5:
        ...
        st.markdown(
            ...
            '<span style="color:#00f5c4">■</span> Risk-On &nbsp;|&nbsp;'
            '<span style="color:#f5a623">■</span> Transitional &nbsp;|&nbsp;'
