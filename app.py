...
def score_vix(v):
    ...
    if v < 15: return 1     # calma vera
    if v < 25: return 0.5   # attenzione
    return 0                # paura
...
def score_hyg_lqd(ratio):
    """HYG/LQD: ratio alto = HY regge = risk-on (bullish). Ratio basso = stress creditizio."""
    if ratio is None: return 0
    # Range storico tipico: 0.82–0.92
    if ratio > 0.88: return 1    # risk-on, spread compressi
    if ratio > 0.84: return 0.5  # neutrale
    return 0                     # stress creditizio
...
with tab2:
    ...
    def _interp_row(name, val, bull_thresh, bear_thresh, bull_txt, bear_txt, neutral_txt):
        ...
        return f"""
        <tr>
          <td style="padding:8px 12px;font-family:Space Mono;font-size:0.7rem;color:#c8d8e8;border-bottom:1px solid #1c2a3a">{name}</td>
          <td style="padding:8px 12px;font-family:Syne;font-size:0.85rem;font-weight:700;color:{zone_col};border-bottom:1px solid #1c2a3a">{val}%</td>
          <td style="padding:8px 12px;font-size:0.65rem;color:{zone_col};border-bottom:1px solid #1c2a3a;letter-spacing:1px">{zone_ico}</td>
          <td style="padding:8px 12px;font-size:0.65rem;color:#8ab0c8;border-bottom:1px solid #1c2a3a">{txt}</td>
        </tr>"""
    ...
    <div style="border:1px solid #1c2a3a;border-radius:4px;overflow:hidden">
      <table style="width:100%;border-collapse:collapse;background:#080e14">
        ...
        <tfoot>
          <tr style="background:#0e1420">
            <td colspan="3" style="padding:8px 12px;font-size:0.62rem;color:#7a9ab0;font-family:Space Mono">
              BREADTH COMPOSITE
            </td>
            <td style="padding:8px 12px">
              <span style="background:{'#0a1a14' if b_label=='BULL' else ('#1a0a0a' if b_label=='BEAR' else '#1a150a')};
                           color:{b_color};border:1px solid {b_color};
...
with tab3:
    ...
    with c3:
        fig_pcr_g = gauge(active_pcr if active_pcr else 0.85, f"Put/Call · {pcr_source}", 0.4, 1.8,
                           thresholds=[21, 50], unit="x", fmt=".2f", invert=True)
    ...
    # ── HYG/LQD Credit Spread — nel tab Sentiment ──────────
    st.markdown('<div class="section-label" style="margin-top:24px">HYG / LQD · Credit Spread Proxy</div>', unsafe_allow_html=True)
    c_hyg1, c_hyg2 = st.columns([1, 2])
    with c_hyg1:
        hl_gauge_val = hyg_lqd_last if hyg_lqd_last else 0.86
        # Alto = bond HY reggono = risk-on → invert=False (alto=verde)
        fig_hl_g = gauge(hl_gauge_val, "HYG/LQD · Credit Ratio", 0.80, 0.94,
                          thresholds=[28, 57], unit="x", fmt=".4f", invert=False)
        st.plotly_chart(fig_hl_g, use_container_width=True, config={"displayModeBar": False})
        if hyg_lqd_last:
            if hyg_lqd_last > 0.88:
                hl_label, hl_msg, hl_col = "🟢 RISK-ON", "Spread compressi · HY bond reggono · no stress sistemico", CYAN
            elif hyg_lqd_last > 0.84:
                hl_label, hl_msg, hl_col = "🟡 NEUTRALE", "Spread in moderato allargamento · monitorare il trend", AMBER
            else:
            ...
            <div style="background:#0e1420;border:1px solid #1c2a3a;padding:12px;
              ...
              Ratio scende = spread si allarga = risk-off</span>
            </div>""", unsafe_allow_html=True)
    with c_hyg2:
        if hyg_lqd is not None and len(hyg_lqd) > 5:
            ...
            st.info("HYG/LQD: dati non ancora disponibili — verifica connessione.")
