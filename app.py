...
# ══════════════════════════════════════════════
with tab5:
    st.markdown('<div class="section-label">Signal Backtester — Inversione su Ribassi · 10-Day Forward Return</div>', unsafe_allow_html=True)

    # ── Fetch longer history for backtest (5y default)
    @st.cache_data(ttl=86400, show_spinner=False)
    def fetch_backtest_data():
        tickers = ["SPY", "^VIX", "^VIX3M", "^CPC", "^PCCE"]
        out = {}
        for t in tickers:
            try:
                ...
                if not df.empty:
                    ...
                    if isinstance(col, pd.DataFrame):
                        col = col.iloc[:, 0]
                    out[t] = col.squeeze().dropna()
            except Exception:
                pass
        return out

    with st.spinner("Caricamento dati storici (5 anni)…"):
        bt = fetch_backtest_data()

    spy_bt  = bt.get("SPY")
    vix_bt  = bt.get("^VIX")
    vix3m_bt = bt.get("^VIX3M")
    # P/C: prova ^CPC, fallback ^PCCE
    pcr_bt = bt.get("^CPC")
    if pcr_bt is None or len(pcr_bt) < 50:
        pcr_bt = bt.get("^PCCE")

    if spy_bt is None or vix_bt is None:
        st.error("Dati insufficienti per il backtest. Verifica la connessione.")
    else:
        FORWARD_DAYS = 10

        # ── Align everything to SPY index
        def align_to_spy(series, spy):
            if series is None:
                return None
            s, _ = series.align(spy, join="right", fill_value=np.nan)
            return pd.Series(s.values, index=spy.index).ffill()

        spy_r  = pd.Series(spy_bt.values, index=spy_bt.index)
        vix_r  = align_to_spy(vix_bt, spy_r)
        vix3m_r = align_to_spy(vix3m_bt, spy_r)
        pcr_r  = align_to_spy(pcr_bt, spy_r)

        # Forward return
        fwd = spy_r.pct_change(FORWARD_DAYS).shift(-FORWARD_DAYS) * 100

        # ─────────────────────────────
        #  SIGNAL DETECTION FUNCTIONS
        # ─────────────────────────────

        def signal_vix_reversion(vix, window=5, spike_thresh=25.0):
            """VIX > spike_thresh, then closes below its own 5d MA → reversion signal."""
            vix_ma = vix.rolling(window).mean()
            was_spiked = (vix > spike_thresh).rolling(window).max().astype(bool)
            cross_down = (vix < vix_ma) & (vix.shift(1) >= vix_ma.shift(1))
            return was_spiked & cross_down

        def signal_vix3m_normalize(vix, vix3m):
            """VIX3M/VIX < 1 (inverted), poi torna > 1 → stress normalizzato."""
            if vix3m is None:
                return pd.Series(False, index=vix.index)
            ratio = vix3m / vix
            inverted_prev = ratio.shift(1) < 1.0
            normalized_now = ratio >= 1.0
            return inverted_prev & normalized_now

        def signal_pcr_reversion(pcr, spike_thresh=1.15, window=3):
            """P/C > 1.15 per almeno window giorni, poi scende sotto 1.0."""
            if pcr is None:
                return pd.Series(False, index=spy_r.index)
            pcr_a = pcr.reindex(spy_r.index).ffill()
            was_high = (pcr_a > spike_thresh).rolling(window, min_periods=window).min().astype(bool)
            cross_down = (pcr_a < 1.0) & (pcr_a.shift(1) >= 1.0)
            return was_high & cross_down

        sig_vix  = signal_vix_reversion(vix_r)
        sig_norm = signal_vix3m_normalize(vix_r, vix3m_r)
        sig_pcr  = signal_pcr_reversion(pcr_r)

        signals = {
            "VIX Reversion (>25→↓MA5)": (sig_vix,  "#ff4d6d", "triangle-up"),
            "VIX3M/VIX Normalize (<1→≥1)": (sig_norm, "#4da6ff", "diamond"),
            "P/C Reversion (>1.15→<1.0)": (sig_pcr,  "#f5a623", "star"),
        }
        ...
        # ─────────────────────────────
        stats_rows = []
        for name, (sig, color, marker) in signals.items():
            dates = spy_r.index[sig.fillna(False).values]
            n = len(dates)
            if n == 0:
                stats_rows.append({"Signal": name, "N eventi": 0,
                                    "Win Rate": "—", "Avg Fwd Return": "—",
                                    "Median": "—", "Best": "—", "Worst": "—"})
                continue
            returns = fwd[sig.fillna(False)].dropna()
            wr = (returns > 0).mean() * 100
            stats_rows.append({
                "Signal": name,
                "N eventi": n,
                "Win Rate": f"{wr:.0f}%",
                "Avg Fwd Return": f"{returns.mean():.1f}%",
                "Median": f"{returns.median():.1f}%",
                "Best": f"+{returns.max():.1f}%",
                "Worst": f"{returns.min():.1f}%",
            })

        df_stats = pd.DataFrame(stats_rows)

        # Metric summary tiles
        st.markdown('<div class="section-label">Statistiche Segnali — SPY Forward +10 giorni (5 anni)</div>', unsafe_allow_html=True)
        cols_s = st.columns(3)
        colors_map = {
            "VIX Reversion (>25→↓MA5)": ("red", RED),
            "VIX3M/VIX Normalize (<1→≥1)": ("blue", BLUE),
            "P/C Reversion (>1.15→<1.0)": ("amber", AMBER),
        }
        for i, row in df_stats.iterrows():
            cc, hx = colors_map.get(row["Signal"], ("", CYAN))
            with cols_s[i]:
                wr_num = float(row["Win Rate"].replace("%","")) if row["Win Rate"] != "—" else 0
                pill_l = "BULL" if wr_num >= 60 else ("NEUTRAL" if wr_num >= 45 else "BEAR")
                html = f"""
                <div class="metric-tile {cc}">
                  <div class="metric-label">{row['Signal']}</div>
                  <div style="font-family:Syne;font-size:2.4rem;font-weight:700;color:{hx};line-height:1.1">{row['Win Rate']}</div>
                  <div style="font-size:0.65rem;color:#c8d8e8;margin-top:4px">win rate &nbsp;·&nbsp; {row['N eventi']} eventi</div>
                  <div style="font-size:0.7rem;color:#c8d8e8;margin-top:6px">
                    Avg: <b>{row['Avg Fwd Return']}</b> &nbsp;|&nbsp;
                    Med: <b>{row['Median']}</b>
                  </div>
                  <div style="font-size:0.65rem;color:#4a6070;margin-top:4px">
                    Best {row['Best']} &nbsp;/&nbsp; Worst {row['Worst']}
                  </div>
                  <div style="margin-top:8px">{signal_pill(pill_l)}</div>
                </div>"""
                st.markdown(html, unsafe_allow_html=True)

        # ─────────────────────────────
        #  MAIN CHART: SPY + signals
        # ─────────────────────────────
        st.markdown('<div class="section-label">SPY — Segnali di Inversione Marcati sul Grafico</div>', unsafe_allow_html=True)

        # Signal selector
        selected_sigs = st.multiselect(
            "Segnali da visualizzare",
            list(signals.keys()),
            default=list(signals.keys()),
        ...
        fig_bt = make_subplots(
            ...
            subplot_titles=("SPY Price", "VIX", "VIX3M/VIX Ratio"),
            row_heights=[0.55, 0.25, 0.20],
        )

        # SPY line
        fig_bt.add_trace(go.Scatter(
            x=spy_r.index, y=spy_r.values, name="SPY",
            line=dict(color=CYAN, width=1.5)), row=1, col=1)

        # 200d MA
        ma200_bt = spy_r.rolling(200).mean()
        fig_bt.add_trace(go.Scatter(
            x=ma200_bt.index, y=ma200_bt.values, name="200d MA",
            line=dict(color=AMBER, width=1, dash="dot")), row=1, col=1)

        # Signal markers on SPY
        for name, (sig, color, marker_sym) in signals.items():
            if name not in selected_sigs:
                continue
            sig_dates = spy_r.index[sig.fillna(False).values]
            sig_prices = spy_r[sig.fillna(False)]
            if len(sig_dates) == 0:
                continue

            # Compute forward return for hover
            hover_texts = []
            for d in sig_dates:
                if d in fwd.index and not np.isnan(fwd[d]):
                    hover_texts.append(f"{name}<br>Date: {d.date()}<br>Fwd +10d: {fwd[d]:.1f}%")
                else:
                    hover_texts.append(f"{name}<br>Date: {d.date()}")

            fig_bt.add_trace(go.Scatter(
                x=sig_dates,
                y=sig_prices.values * 0.985,   # slightly below price
                mode="markers",
                name=name,
                marker=dict(
                    symbol=marker_sym,
            ...
            # Vertical shading: entry → +10 days
            for d in sig_dates:
                try:
                    end_d = spy_r.index[spy_r.index.get_loc(d) + FORWARD_DAYS]
                    price_entry = float(spy_r[d])
                    price_exit  = float(spy_r[end_d])
                    fill_col = "rgba(0,245,196,0.06)" if price_exit >= price_entry else "rgba(255,77,109,0.06)"
                    fig_bt.add_vrect(
                        x0=d, x1=end_d,
                        fillcolor=fill_col,
        ...
        # VIX panel
        fig_bt.add_trace(go.Scatter(
            x=vix_r.index, y=vix_r.values, name="VIX",
            line=dict(color=RED, width=1.2),
            fill="tozeroy", fillcolor="rgba(255,77,109,0.07)"),
            row=2, col=1)
        fig_bt.add_hline(y=25, line_dash="dot", line_color=RED,   line_width=1, row=2, col=1)
        fig_bt.add_hline(y=20, line_dash="dot", line_color=AMBER,  line_width=1, row=2, col=1)

        # VIX3M/VIX panel
        if vix3m_r is not None:
            ratio_bt = vix3m_r / vix_r
            fig_bt.add_trace(go.Scatter(
                x=ratio_bt.index, y=ratio_bt.values, name="VIX3M/VIX",
                line=dict(color=BLUE, width=1.2)),
                row=3, col=1)
            fig_bt.add_hline(y=1.0, line_dash="dot", line_color=TEXT_COL, line_width=1, row=3, col=1)
        ...
        fig_bt.update_layout(
            ...
            xaxis2=dict(gridcolor=GRID_COL),
            xaxis3=dict(gridcolor=GRID_COL),
            yaxis2=dict(gridcolor=GRID_COL, title=dict(text="VIX", font=dict(size=9))),
            yaxis3=dict(gridcolor=GRID_COL, title=dict(text="Ratio", font=dict(size=9))),
        )
        st.plotly_chart(fig_bt, use_container_width=True, config={"displayModeBar": True})

        # ─────────────────────────────
        #  DISTRIBUTION OF RETURNS
        # ─────────────────────────────
        st.markdown('<div class="section-label">Distribuzione Ritorni Forward per Segnale</div>', unsafe_allow_html=True)
        fig_dist = go.Figure()
        for name, (sig, color, _) in signals.items():
            if name not in selected_sigs:
                continue
            returns = fwd[sig.fillna(False)].dropna()
            if len(returns) < 3:
            ...
            fig_dist.add_trace(go.Violin(
                x=[name.split("(")[0].strip()] * len(returns),
                y=returns.values,
        ...
        fig_dist.add_hline(y=0, line_dash="dot", line_color=TEXT_COL, line_width=1)
        fig_dist.update_layout(
            **base_layout("Distribuzione Ritorni +10 giorni (%)", 320),
            violinmode="group",
        )
        fig_dist.update_layout(paper_bgcolor=PAPER_BG, plot_bgcolor=PLOT_BG)
        st.plotly_chart(fig_dist, use_container_width=True, config={"displayModeBar": False})

        # ─────────────────────────────
        #  RECENT SIGNALS TABLE
        # ─────────────────────────────
        st.markdown('<div class="section-label">Ultimi 10 Segnali Rilevati</div>', unsafe_allow_html=True)
        recent_rows = []
        for name, (sig, color, _) in signals.items():
            sig_dates = spy_r.index[sig.fillna(False).values]
            for d in sig_dates[-10:]:
                try:
                    loc = spy_r.index.get_loc(d)
                    end_loc = loc + FORWARD_DAYS
                    fwd_ret = None
                    if end_loc < len(spy_r):
                        fwd_ret = (float(spy_r.iloc[end_loc]) / float(spy_r.iloc[loc]) - 1) * 100
                    recent_rows.append({
                        "Data Segnale": d.date(),
                        "Tipo": name.split("(")[0].strip(),
                        "SPY Entry": f"${float(spy_r.iloc[loc]):.2f}",
                        "VIX": f"{float(vix_r.iloc[loc]):.1f}" if not np.isnan(vix_r.iloc[loc]) else "—",
                        f"+{FORWARD_DAYS}d Return": f"{fwd_ret:+.1f}%" if fwd_ret is not None else "In corso",
                        "Esito": ("✅ Win" if fwd_ret and fwd_ret > 0
                                  else ("❌ Loss" if fwd_ret is not None else "⏳")),
                    })
                except Exception:
                    pass
        if recent_rows:
            df_recent = pd.DataFrame(recent_rows).sort_values("Data Segnale", ascending=False).head(15)
